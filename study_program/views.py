...
import csv
from .models import StudyProgram, Professor, AssessmentResult, Committee, AUN, AvailableTime, Issue, Comment, RecommendedCommittee
...
def combine_recommendation(request):
    #Delete all existing object
    RecommendedCommittee.objects.all().delete()
    ...
    for new_committee in recommendation_committee_knn_list:
        for committee in recommendation_committee_pearson_list:
            ...
            if(new_committee[0] == committee[0]):
                ...
                rc = RecommendedCommittee.objects.create(program_name = program[0], committee1 = name1, committee2 = name2, committee3 = name3, committee4 = name4)

                #print(committee[0], committee[1], committee[2], new_committee[1], new_committee[2])

                break

    return redirect('committee_recommendation')
...
def committee_recommendation(request, page_number = 1):
    aee_list = RecommendedCommittee.objects.all().order_by('id')
    temp_list = []
    ...
    total_recommendation_committee_dict = aee_list
...
def assessment_result(request, assessment_id):
    ...
    #print(aun_result)
    return render(request, 'assessment/assessment_result.html', {'assessment_result': detail, 'commitee_list':commitee_list, 'assessment_id': assessment_id,'aun_result':aun_result,'aun_id':assessment_id})
...
def edit_assessment_result(request, assessment_id):
    ...
    context = {
        'form':form, 'aun_id':assessment_id
    }
    return render(request, "assessment/edit_assessment_result.html", context)

@user_passes_test(lambda u: u.is_superuser, login_url='all_assessment')
@login_required(login_url="login")
def edit_aun(request, aun_id):
    aun = get_object_or_404(AUN, pk=aun_id)
    if request.method == "POST":
    ...
    return render(request, "assessment/edit_aun.html", context)
...
def export_studyprogram_csv(request):
    ...
    writer.writerow(['Code','Program', 'Program Status', 'Degree & Major', 'Program Collaborations with Other Insitues', 'docs', 'pdf_docs_link','responsible committee'])
    ...
    for studyProgram in studyPrograms:
        response_list = [studyProgram.code, studyProgram.name, studyProgram.program_status, studyProgram.degree_and_major, studyProgram.collaboration_with_other_institues, studyProgram.pdf_docs, studyProgram.pdf_docs_link]
        try:
            responsible_prof_list = []
            for item in studyProgram.responsible_professors.all():
                responsible_prof_list.append(item.name_surname)
            response_list = response_list + responsible_prof_list
...
def export_faculty_studyprogram_csv(request):

    response = HttpResponse(content_type='text/xlsx')
    response['Content-Disposition'] = 'attachment; filename="facultystudyprogram.csv"'
    response.write(u'\ufeff'.encode('utf8'))
...
def export_committee_recommendation_csv(request):
    ...
    aee_list = RecommendedCommittee.objects.all()    
    for item in aee_list:
        response_list = [item.program_name,item.committee1,item.committee2,item.committee3,item.committee4]
        writer.writerow(response_list)
    return response
